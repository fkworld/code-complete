...
async function genApiDefinesExport(apiConfigs: Array<ApiConfig>) {
  ...
  fs.writeFileSync(
    outputFile,
    [
      `// This file was auto generated by ${SCRIPT_FILENAME}`,
      "// @ts-nocheck",
      apiConfigs.map((apiConfig) => {
        return `export type { paths as ${upperFirst(camelCase(`Api ${apiConfig.name} Defines`))} } from "./api-${kebabCase(apiConfig.name)}.ts"`;
      }),
    ].join("\n"),
  );
  console.log(chalk.green(`Gen ${outputFileName} success! ${(performance.now() - startTime).toFixed(1)}ms`));
